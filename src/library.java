...
//}
    ...
    public Students searchStd(String name) {
        for (Students s : ListEtudiants) {
            {
    ...
    void supprimerReservation() {
        ...
        Students std = searchStd(Rechstd);
//        std.reservations.removeIf(stude -> stude.student.Name.equalsIgnoreCase(Rechstd));
        System.out.println("entrer le nom du livre");
        String RechBk = input.next();
        Books bk = searchBook(RechBk);
        std.reservations.remove(bk);
        bk.student = null;
//        std.reservations.removeIf(bks -> bks.Title.equalsIgnoreCase(RechBk));
